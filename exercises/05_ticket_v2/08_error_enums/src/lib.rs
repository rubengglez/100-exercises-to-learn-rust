...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(err) => match err {
            TicketNewError::WrongTitle(info) => panic!("{}", info),
            TicketNewError::WrongDescription(_) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::WrongTitle(
        ...
        if description.is_empty() {
            return Err(TicketNewError::WrongDescription(
