...
pub enum TicketRescriptionErrors {
...
impl TryFrom<String> for TicketDescription {
    type Error = TicketRescriptionErrors;

    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(TicketRescriptionErrors::TitleEmpty);
        }
         if value.len() > 500 {
            return Err(TicketRescriptionErrors::TitleTooLong);
        }
